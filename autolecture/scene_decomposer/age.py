...
#     Args:
#         code: The generated Python string containing the Manim scene class.
#         class_name: The exact name of the Manim Scene class to render (e.g., 'LinearScanProblem').
#     """
#     # 1. Write the code generated by the agent to a file
#     python_file = "generated_scene.py"
#     with open(python_file, "w") as f:
#         f.write(code)
        
#     # 2. Run Manim to compile the video (-ql for low quality / fast render)
#     cmd = f"manim -ql {python_file} {class_name}"
#     result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    
#     if result.returncode != 0:
#          return f"Compilation failed! Error log:\n{result.stderr}"
         
#     # 3. Move the compiled video to the ADK artifacts folder
#     # Manim's default output path for low quality is typically in media/videos/
...
#          return "Success! Video compiled. You can now view it in the 'Artifacts' tab in ADK Web UI."
         
#     return "Code compiled, but could not find the output video file."


# execution_agent = Agent(
#     model='gemini-2.5-flash',
#     name='execution_agent',
#     description='Takes the generated Manim code and executes it to make a video.',
#     instruction="""Identify the Python code block and class name generated by the previous agent. 
#     Call the compile_manim_and_save_artifact tool to compile the video. Do not return any raw code yourself.""",
#     tools=[compile_manim_and_save_artifact]
# )
